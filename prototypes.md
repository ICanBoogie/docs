...
If the package is bound to [ICanBoogie][] using [icanboogie/bind-prototype][], prototype methods may
be defined using `prototype` configuration fragments:
...
An array representation of an `Prototyped` instance can be obtained using the `to_array()` method.
Only public and façade properties are exported.
